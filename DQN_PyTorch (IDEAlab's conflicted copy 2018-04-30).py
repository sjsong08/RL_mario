...
def OutputToAction3(output):
    actions={ # A: jump , B: run
        ...
        '1' :[[0,0,1,0,0,0], 'Down'],
        '2' :[[0,1,0,0,0,0], 'Left'],
        '3' :[[0,1,0,0,1,0], 'Left+jump'],
        #'5' :[[0,1,0,0,0,1], 'Left+run'],
        '4' :[[0,1,0,0,1,1], 'Left+jump,run'],
        '5' :[[0,0,0,1,0,0], 'Right'],
        '6' :[[0,0,0,1,1,0], 'Right+jump'],
        #'9' :[[0,0,0,1,0,1], 'Right+run'],
        '7':[[0,0,0,1,1,1], 'Right+jump,run'],
        '8':[[0,0,0,0,1,0], 'jump'],
        #'12':[[0,0,0,0,1,1], 'jump,run']
    }
...
def main():
    if train:
        ...
        if retrain:
            mainDQN.load_state_dict(torch.load('checkpoints/mainDQN-1.pt'))
            targetDQN.load_state_dict(mainDQN.state_dict())
        ...
        for episode in range(init_episode, max_episode):
            ...
            while not done:
                ...
                if np.random.rand(1) < e or state is None or state.size == 1 or step_count <=10:
                    ...
                    print("random: %15s" %(action_name), end=" ")
                    ...
                    print("predicted: %15s" %(action_name), end=" ")
                ...
                for n in range(len(action)):
                    ...
                    if done:
                        print('DONE!! final distance: ',info['distance'] )
                        break
                print("distance: %3d" %(info['distance']), end=" ")
                print('sys_r: %2d' %(reward), end=" ")
                state_buffer.append(next_state)
                output_buffer.append(action)

                prev_distance = distance
                distance = info['distance']
                dist_20.append(distance)
                ...
                #reward = got_distance/2. #got_score/50 + got_distance/30
                if reward == 1:
                    reward = 0
                reward = int(np.clip(reward, -2, 3))/2.
                
                if output >= 2 and output <=4:
                    reward = int(np.clip(reward, -100, 0))
                
                if output >= 5 and output <=7:
                    reward = int(np.clip(reward, 0, 100))
                
                if step_count > 30:
                    dist_20.pop(0)
                    if np.max(np.abs(dist_20[-30:] - np.median(dist_20[-30:-1]))) < 10:
                        reward = -5
                
                
                print('my_r: %2d' %(reward))
                if done:
                    if TIME > 1:
                        reward -= 10

                    DISTANCE.append((episode, distance, e))
                ...
                if step_count>=10:
                    acc_state = [state_buffer[-2*k-2] for k in range(5)]
                    state_buffer.popleft()
                    acc_state = np.reshape(acc_state, input_size[:3])
                    acc_next_state = [state_buffer[-2*k-1] for k in range(5)]
                    acc_next_state = np.reshape(acc_next_state, input_size[:3])

                    output_seq = [output_buffer[-2*k-2] for k in range(5)]
                    output_next_seq = [output_buffer[-2*k-2] for k in range(5)]
            ...
            if (episode+1) % 50 == 0:
                ...
                torch.save(mainDQN.state_dict(), 'checkpoints/SJ_level1-2_01.pt')
                print('model saved')
